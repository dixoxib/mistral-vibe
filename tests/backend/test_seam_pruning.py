...
def _seam_user(text: str = " summary") -> LLMMessage:
    return LLMMessage(role=Role.user, content=SEAM_MARKER + text, injected=True)


def _seam_assistant(text: str = " summary") -> LLMMessage:
    return LLMMessage(role=Role.assistant, content=SEAM_MARKER + text, injected=True)
...
class TestSeamPruning:
    ...
    def test_prune_idx_finds_user_seam(self) -> None:
        messages = [
            _user("hi"),
            _tool("old", "old result"),
            _seam_user(),
    ...
    def test_prune_idx_finds_assistant_seam(self) -> None:
        messages = [
            _user("hi"),
            _tool("old", "old result"),
            _user("pseudo question", injected=True),
            _seam_assistant(),
            _tool("recent", "recent result"),
        ]
        converted = _prepare(messages, prune_idx=3)
        assert converted[1]["content"] == "[tool result for old pruned]"
    ...
    def test_middleware_action_seam_exists(self) -> None:
        from vibe.core.middleware import MiddlewareAction
        assert hasattr(MiddlewareAction, "SEAM")
